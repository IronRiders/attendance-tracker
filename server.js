...
function initializeMeetingScheduler() {
  ...
  db.getMeetingSchedules((err, schedules) => {
    ...
    schedules.forEach((schedule) => {
      ...
      console.log(
        `Meeting end auto-logout scheduled for day ${day_of_week}, session ${session_number} at ${logoutTime} (15 minutes after meeting end at ${end_time})`,
      );
    ...
    checkedInMembers.forEach((member) => {
      db.recordAutoLogout(member.id, (err) => {
        ...
        if (err) {
          ...
          console.log(
            `Meeting-end auto-logged out: ${member.name} (${member.barcode})`,
          );
        }
        ...
        if (processed === checkedInMembers.length) {
          console.log(
            `Meeting-end auto-logout completed: ${processed - errors} successful, ${errors} errors`,
          );
        }
      });
    });
...
app.use(
  session({
    secret:
      process.env.SESSION_SECRET || "attendance-system-secret-" + Math.random(),
    resave: false,
...
app.post("/api/members/import", requireAuth, (req, res) => {
  ...
  for (const member of members) {
    if (!member.name || !member.barcode) {
      return res
        .status(400)
        .json({ error: "Each member must have both name and barcode" });
    }
  }
          ...
          if (!activeSession) {
            ...
            db.getNextMeetingSession((err, nextSession) => {
              ...
              if (nextSession) {
                ...
                const sessionName =
                  nextSession.session_name || `Session ${nextSession.session}`;
  ...
  db.getAttendanceByDateRange(start, end, (err, records) => {
    ...
    Object.keys(summary).forEach((memberId) => {
      ...
      Object.keys(dailyRecords).forEach((date) => {
        const dayRecords = dailyRecords[date].sort(
...
app.put("/api/meeting-schedules", requireAuth, (req, res) => {
  ...
  db.getMeetingSchedules((err, currentSchedules) => {
    ...
    const newScheduleKeys = new Set(
      schedules.map((s) => `${s.dayOfWeek}-${s.sessionNumber}`),
    );
    const schedulesToDelete = currentSchedules.filter(
      (current) =>
        !newScheduleKeys.has(
          `${current.day_of_week}-${current.session_number}`,
        ),
    );
    ...
    const checkComplete = () => {
      ...
      if (completed === totalOperations) {
        if (errors === 0) {
          ...
          res.json({
            success: true,
            message: "Meeting schedules updated successfully",
          });
        } else {
      ...
      db.deleteMeetingSchedule(
        ...
        (err) => {
          if (err) {
            ...
            console.log(
              `Deleted schedule for day ${schedule.day_of_week}, session ${schedule.session_number}`,
            );
          }
          checkComplete();
        },
    ...
    schedules.forEach((schedule) => {
      const { dayOfWeek, sessionNumber, startTime, endTime, sessionName } =
        schedule;

      db.updateMeetingSchedule(
        dayOfWeek,
        sessionNumber,
        startTime,
        endTime,
  ...
  db.deleteMeetingSchedule(dayOfWeek, sessionNumber, (err) => {
    if (err) {
      ...
      if (meetingSchedulers[schedulerId]) {
        ...
        console.log(
          `Removed auto-logout scheduler for day ${dayOfWeek}, session ${sessionNumber}`,
        );
      }

      res.json({
        success: true,
        message: "Session deleted and auto-logout removed",
      });
    }
  });
});
...
app.listen(PORT, () => {
  ...
  console.log(
    `Admin credentials loaded from environment variables: username=${process.env.ADMIN_USERNAME || "admin"}`,
  );
});
